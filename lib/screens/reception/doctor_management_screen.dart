...
String formatExperience(String? exp) {
  ...
  Future<void> _launchEmail(String email) async {
    final Uri emailUri = Uri(
      scheme: 'mailto',
      path: email,
      query: 'subject=Query regarding your appointment&body=Dear Doctor,',
    );
    if (await canLaunchUrl(emailUri)) {
      await launchUrl(emailUri);
    } else {
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const AppText("Admin / Dashboard", color: Colors.grey, fontSize: 13),
            const SizedBox(height: 15),
            Obx(() => _DoctorStatCard(
  ...
  Widget _buildToolbar(DoctorManagementController controller) {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // Added the "Available doctors" heading here as per image
          const AppText("Available doctors", 
                ...
                child: Obx(() => DropdownButton<String>(
                      ...
                      items: ["All", "ACTIVE", "INACTIVE"]
                          .map((val) => DropdownMenuItem(
                              value: val, child: AppText(val, fontSize: 14)))
                          .toList(),
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          // Department pill-style logic can be added here if needed
          Expanded(flex: 2, child: AppText(doc.department)),
          Expanded(flex: 2, child: AppText (formatExperience(doc.experience))),
