            ...
            child: _isControllerInitialized 
                ? Obx(() {
                    ...
                    return _InvestigationTableCard(
                      isMobile: isMobile,
                      investigations: _controller.investigations,
                      patient: widget.patient,               // ← pass full patient object
                      doctorMongoId: widget.doctorMongoId,   // ← pass doctor id
                      controller: _controller,
                    );
                  })
                : const Center(
...
class _InvestigationTableCard extends StatelessWidget {
  final bool isMobile;
  final List<InvestigationModel> investigations;
  final PatientModel patient;               // new
  final String? doctorMongoId;              // new
  final InvestigationController controller;
  ...
  const _InvestigationTableCard({
    ...
    required this.doctorMongoId,             // new
    required this.controller,
  });
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              return _TableDataRow(
                index: index + 1,
                investigation: investigations[index],
                patient: patient,                     // ← pass down
                doctorMongoId: doctorMongoId,         // ← pass down
                controller: controller,
              );
            },
          ),
...
class _TableDataRow extends StatelessWidget {
      ...
      child: Row(
        children: [
          ...
          Expanded(
            flex: 1,
            child: Center(
              child: AppText('$index.', fontSize: 13),
            ),
          ),

          // Investigation - centered (icon + text)
          Expanded(
            flex: 3,
            child: Center(
              child: Row(
                ...
                children: [
                  const Icon(Icons.medical_services_outlined, size: 18, color: AppColors.info),
                  const SizedBox(width: 6),
          ...
          // Date & Time - centered vertically and horizontally
          Expanded(
            flex: 3,
            child: Center(
              child: Column(
                ...
                children: [
                  AppText(date, fontSize: 12),
                  const SizedBox(height: 2),
                  AppText(time, fontSize: 11, color: AppColors.greyText),
                ],
              ),
            ),
          ),

          // Status - already centered
          Expanded(
            flex: 2,
            child: Center(
              child: _TagChip(
          ...
          // Action - already centered (icons row)
          Expanded(
            flex: 2,
            child: Row(
              mainAxisAlignment: MainAxisAlignment.center,
                ...
                _ActionIcon(
                  ...
                  onTap: isDeleting ? null : () {
                    showDialog(
                      ...
                      controller.getInvestigationsByPatientId(patient.id);
                    });
                  },
                ),
                isDeleting
                    ...
                    : _ActionIcon(
                        ...
                        color: Colors.red,
                        onTap: () {
                          _showDeleteConfirmation(context, investigation, patient.id, controller);
                        },
                      ),
              ],
            ),
  ...
  void _showDeleteConfirmation(
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const AppText('Delete Investigation'),
        content: AppText('Are you sure you want to delete this investigation for ${investigation.investigationType}?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const AppText('Cancel'),
